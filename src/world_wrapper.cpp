...
void World_wrapper::initialise_origin_orientation(ww::World_wrapper& world_wrapper,const std::string& root_link_name){
    ...
    while (!world_wrapper.isInitialised()){
        ...
        try{
            ...
            for(std::size_t i = 0; i < world_wrapper.get_number_objects();i++){
                link_name = world_wrapper.get_link_name(i);
                listener.lookupTransform("/" + root_link_name, "/" + link_name, ros::Time(0), transform);
